...
struct fsm_state_t* fsm_add_sub_state(struct fsm_state_t* parent, const char* name, enter_func_t enter,
                                    leave_func_t leave, void* userdata)
{   
    struct fsm_state_t* state;
    ...
    state->fsm      = parent->fsm;
    state->enter    = enter;
    state->leave    = leave;
    state->priv_data= userdata;
    state->next     = parent->substates;
    parent->substates = state;
